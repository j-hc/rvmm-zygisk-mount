...
static char* PROCS_MAP = nullptr;

static bool getMountSrcDst(const char* proc, const char** src, const char** dst) {
...
static size_t readCompanion(int fd) {
    off_t size;
    ...
    while (size_read < size) {
        ssize_t ret = read(fd, PROCS_MAP + size_read, size - size_read);
        if (ret < 0) {
            LOGD("ERROR read: %s", strerror(errno));
            return 0;
        } else {
...
static void run(const char* proc, zygisk::Api* api) {
    int fd = api->connectCompanion();
    size_t comp_len = readCompanion(fd);
    close(fd);
